...
export function MobileNav() {
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40"
      style={{ background: 'rgba(255,255,255,0.97)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', borderTop: '1px solid rgba(0,0,0,0.07)' }}>
          ...
          if (primary) {
            return (
              <Link key={href} href={href} className="flex flex-col items-center gap-0.5 -mt-6">
                ...
                <span className="text-xs font-medium mt-1" style={{ color: '#0F4C81' }}>{label}</span>
              </Link>
            );
          }
          ...
          return (
            <Link key={href} href={href} className="flex flex-col items-center gap-0.5 px-2 py-1 rounded-2xl transition-all min-w-[56px]">
              <div className={cn('w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-200',
                ...
                <Icon className={cn('w-5 h-5 transition-colors', isActive ? 'text-primary-600' : 'text-gray-400')} />
              </div>
              <span className={cn('text-xs font-medium transition-colors', isActive ? 'text-primary-600' : 'text-gray-400')}>
                {label}
              </span>
            </Link>
          );
